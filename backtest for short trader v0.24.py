...
def plot_stock(
    ...
    highest_since_entry = None  # 進場後的最高價（用於移動停損）
    ...
    for _, row in df.iterrows():
        ...
        if trailing_stop_enabled and position_shares > 0 and highest_since_entry is not None:
            stop_threshold = highest_since_entry * (1 - trailing_stop_pct / 100)
            trailing_stop_trigger = close <= stop_threshold

        # 1) 停利/停損賣出
        if (row["sell_signal"] or trailing_stop_trigger) and position_shares > 0:
...
def main():
    ...
    frame_mode.grid(row=5, column=1, sticky="w", pady=(5, 0))

    # === 買進數值輸入 ===
    frame_input = tk.Frame(controls)
    frame_input.grid(row=6, column=1, sticky="we", pady=(2, 0))
    ...
    frame_sell_mode.grid(row=7, column=1, sticky="w", pady=(5, 0))

    frame_sell_input = tk.Frame(controls)
    frame_sell_input.grid(row=8, column=1, sticky="we", pady=(2, 0))
    ...
    entry_capital.grid(row=9, column=1, sticky="we", pady=(5, 0))
