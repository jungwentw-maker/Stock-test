...
def plot_stock(
    ...
    name = stock_name_map.get(stock_id) or stock_name_map.get(stock_id.lstrip("0")) or ""
    graphic.plot_backtest_figure(df, stock_id, name, period, show_raff_channels=show_raff_channels)
...
def main():
    ...
    tk.Label(controls, text="停利 / 停損條件：")\
        .grid(row=4, column=0, sticky="nw", padx=(0, 5), pady=(5, 0))

    frame_sell_cond = tk.Frame(controls)
    frame_sell_cond.grid(row=4, column=1, sticky="w", pady=(5, 0))
    ...
    frame_trailing.grid(row=5, column=1, sticky="w", pady=(5, 0))
    ...
    frame_trailing_profit.grid(row=6, column=1, sticky="w", pady=(5, 0))
    ...
    tk.Label(controls, text="買進模式：")\
        .grid(row=7, column=0, sticky="w", padx=(0, 5), pady=(5, 0))

    mode_var = tk.StringVar(value="amount")

    frame_mode = tk.Frame(controls)
    frame_mode.grid(row=7, column=1, sticky="w", pady=(5, 0))

    # === 買進數值輸入 ===
    frame_input = tk.Frame(controls)
    frame_input.grid(row=8, column=1, sticky="we", pady=(2, 0))
    ...
    tk.Label(controls, text="賣出模式：")\
        .grid(row=9, column=0, sticky="w", padx=(0, 5), pady=(5, 0))

    sell_mode_var = tk.StringVar(value="shares")

    frame_sell_mode = tk.Frame(controls)
    frame_sell_mode.grid(row=9, column=1, sticky="w", pady=(5, 0))

    frame_sell_input = tk.Frame(controls)
    frame_sell_input.grid(row=10, column=1, sticky="we", pady=(2, 0))
    ...
    tk.Label(controls, text="初始資金池：")\
        .grid(row=11, column=0, sticky="w", padx=(0, 5), pady=(5, 0))
    entry_capital = tk.Entry(controls)
    entry_capital.insert(0, "100000")
    entry_capital.grid(row=11, column=1, sticky="we", pady=(5, 0))
    ...
    def on_plot():
        try:
            ...
            log_path = write_error_log("on_plot 未捕捉例外（backtest v0.26）")
...
if __name__ == "__main__":
    try:
        ...
        error_log_path = write_error_log("未捕捉例外（backtest v0.26）")
