...
def plot_stock(
    ...
    # === 停利/停損賣出條件：前一日收盤 > 前一日 MA20 且今日收盤 < 今日 MA20 ===
    sell_signal = (
    ...
    for _, row in df.iterrows():
        ...
        # 1) 停利/停損賣出
        if row["sell_signal"] and position_shares > 0:
        ...
        if row["buy_signal"]:
            if mode == "shares":
                ...
                buy_shares = int(amount_cfg // close)
            else:
    ...
    if len(trade_df) > 0:
        ...
        total_buy = trade_df.loc[trade_df["方向"] == "BUY", "金額"].sum()
        total_sell = trade_df.loc[trade_df["方向"] == "SELL", "金額"].sum()
        final_value = position_shares * final_close
        total_realized = trade_df["單筆實現損益"].sum()
        unrealized = (final_close - avg_cost) * position_shares if position_shares > 0 else 0
        total_profit = total_sell - total_buy + final_value  # 現金流 + 期末市值
        ...
        print(f"總報酬（賣出-買入+期末市值）：{total_profit:,.2f}")
        cost_for_pct = total_buy if total_buy > 0 else 1  # 避免除以 0
        profit_pct = (total_profit / cost_for_pct) * 100
...
def main():
    ...
    frame_sell_mode.grid(row=5, column=1, sticky="w", pady=(5, 0))

    frame_sell_input = tk.Frame(controls)
    frame_sell_input.grid(row=6, column=1, sticky="we", pady=(2, 0))
